...
@interface ACSelectMediaView ()<UICollectionViewDelegate, UICollectionViewDataSource, TZImagePickerControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, MWPhotoBrowserDelegate,upImageDelegate,UICollectionViewDelegateFlowLayout>
...
- (void)configureCollectionView {
    ...
    layout.itemSize = CGSizeMake((SCREEN_WIDTH-130)*0.33, (SCREEN_WIDTH-130)*0.33);
    layout.minimumLineSpacing = 0;
    layout.minimumInteritemSpacing = 0;
    layout.sectionInset = UIEdgeInsetsMake(0, 0, 0, 0);
    layout.scrollDirection = UICollectionViewScrollDirectionVertical;
    self.collectionView.alwaysBounceVertical = YES;
    _collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH-130,75) collectionViewLayout:layout];
    [_collectionView registerClass:[ACMediaImageCell class] forCellWithReuseIdentifier:NSStringFromClass([ACMediaImageCell class])];
      [_collectionView registerNib:[UINib nibWithNibName:@"WKUpVideoCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"cellid"];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row ==0&&indexPath.row != _mediaArray.count&&_isMerge ==1) {
        return CGSizeMake((SCREEN_WIDTH-130)*0.66, (SCREEN_WIDTH-130)*0.33);
    }
    return CGSizeMake((SCREEN_WIDTH-130)*0.33, (SCREEN_WIDTH-130)*0.33);
}
...
//    _collectionView.height = allImageCount == 0 ? 0 : maxRow * ACMedia_ScreenWidth/3;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if ([mediaType isEqualToString:@"public.movie"]) {
        ...
        [[ACMediaManager manager] getVideoPathFromURL:videoAssetURL PHAsset:asset enableSave:NO completion:^(NSString *name, UIImage *screenshot, id pathData) {
            ...
            __weak typeof(self) weakself= self;
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
                [WKHttpTool uploadVideoWithURLString:self.uploadUrl parameters:self.dic images:screenshot date:pathData success:^(id responseObject) {
                    //NSLog(@"resssss=%@",responseObject);
                 dispatch_async(dispatch_get_main_queue(), ^{
                     if ([[responseObject objectForKey:@"flag"]intValue]) {
                            [_mediaArray addObject:model];
                         [weakself layoutCollection];
                         WKUploadModel *upmodel = [WKUploadModel mj_objectWithKeyValues:responseObject];
//                         NSLog(@"upmodel=%@",upmodel);
//                         NSLog(@"up = %lu",upmodel.fileName);
                         ...
                         NSLog(@"233");
                         weakself.hud.label.text = @"视频时长需大于十秒";
...
-(void)editCoverAction{
    [self.upload selectUserpicSourceWithViewController:self.viewController];
...
-(void)selctedImage:(NSDictionary*)Imgestring{
    if (Imgestring ==nil) {
        self.CoverImageUrl = nil;
         ...
         self.CoverImageUrl = [Imgestring objectForKey:@"videoImage"];
        WKUpVideoCollectionViewCell *cell = (WKUpVideoCollectionViewCell*) [self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:0]];
        [ cell.videoCover sd_setImageWithURL:[NSURL URLWithString:self.CoverImageUrl] placeholderImage:[UIImage imageNamed:@"water"] options:SDWebImageLowPriority|SDWebImageRetryFailed ];
